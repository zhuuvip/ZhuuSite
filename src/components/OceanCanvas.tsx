...
interface Bubble {
  x: number; y: number; radius: number; speed: number; opacity: number;
  swayAmplitude: number; swaySpeed: number; swayOffset: number; color: string;
}
interface Particle {
  x: number; y: number; vx: number; vy: number; opacity: number; radius: number; life: number; maxLife: number;
}
interface Fish {
  x: number; y: number; speed: number; direction: number; waveAmplitude: number;
  waveSpeed: number; waveOffset: number; size: number; opacity: number; color: string; layer: number;
}
...
export default function OceanCanvas() {
  ...
  const drawFish = useCallback((ctx: CanvasRenderingContext2D, fish: Fish, t: number) => {
    ...
    const x = fish.x, y = fish.y + yOff, dir = fish.direction, s = fish.size;
    ctx.save();
    ctx.globalAlpha = fish.opacity;
    if (fish.layer > 0) ctx.filter = `blur(${fish.layer * 1.5}px)`;
    ctx.translate(x, y);
    ctx.scale(dir, 1);
    ctx.beginPath();
  ...
  const drawBubble = useCallback((ctx: CanvasRenderingContext2D, bubble: Bubble, t: number) => {
    ...
    const grad = ctx.createRadialGradient(
      bubble.x + xOff - bubble.radius * 0.3, bubble.y - bubble.radius * 0.3, bubble.radius * 0.1,
      bubble.x + xOff, bubble.y, bubble.radius
    );
  ...
  const drawLightRays = useCallback((ctx: CanvasRenderingContext2D, width: number, t: number) => {
    for (let i = 0; i < 6; i++) {
      const x = (width / 7) * (i + 1) + Math.sin(t * 0.002 + i) * 30;
  ...
  useEffect(() => {
    ...
    bubblesRef.current = Array.from({ length: 35 }, () => { const b = createBubble(canvas); b.y = Math.random() * canvas.height; return b; });
    ...
    const handleClick = (e: MouseEvent) => {
      for (let i = 0; i < 3; i++) ripplesRef.current.push({ x: e.clientX, y: e.clientY, radius: i * 8, maxRadius: 80, opacity: 0.7 - i * 0.15 });
      for (let i = 0; i < 6; i++) bubblesRef.current.push({
        x: e.clientX + (Math.random() - 0.5) * 40, y: e.clientY + (Math.random() - 0.5) * 20,
        radius: Math.random() * 8 + 3, speed: Math.random() * 2 + 1, opacity: Math.random() * 0.7 + 0.3,
        swayAmplitude: Math.random() * 40 + 15, swaySpeed: Math.random() * 0.03 + 0.01,
        swayOffset: Math.random() * Math.PI * 2, color: 'rgba(0,255,255,',
      });
    };
    ...
    const animate = () => {
      ...
      bubblesRef.current.forEach((bubble, i) => {
        ...
        if (dist < 80) { const force = (80 - dist) / 80; bubble.x += (dx / dist) * force * 2; }
        drawBubble(ctx, bubble, t);
        if (bubble.y < -20) bubblesRef.current[i] = createBubble(canvas);
      });
      while (bubblesRef.current.length < 35) bubblesRef.current.push(createBubble(canvas));
      ctx.save(); ctx.globalAlpha = 0.6;
      fishRef.current.forEach((fish, i) => {
        fish.x += fish.speed;
        drawFish(ctx, fish, t);
        if ((fish.direction === 1 && fish.x > canvas.width + 200) || (fish.direction === -1 && fish.x < -200)) fishRef.current[i] = createFish(canvas);
      });
      ctx.restore();
      particlesRef.current.forEach((p, i) => {
        ...
        ctx.fillStyle = `rgba(0,220,255,${opacity})`; ctx.fill();
        if (p.life >= p.maxLife || p.y < 0 || p.y > canvas.height) particlesRef.current[i] = createParticle(canvas);
      });
      drawRipples(ctx);
